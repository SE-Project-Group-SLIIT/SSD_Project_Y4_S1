...
function VehicleList() {
	...
	const deleteVehicle = async (data) => {
		...
		try {
			await deleteVehicleService(data); // Call the deleteVehicleService

			Swal.fire({
				title: "Success!",

				text: "Permanently deleted the Vehicle Record",
			...
			// Optionally, you can reload the vehicle data after successful deletion.

			// Example:

			// const updatedVehicleData = await viewVehicle();
	...
	function searchVehicles(e) {
		...
		axios
			.get(`http://localhost:8070/vehicle/searchV/${search}`)
			.then((res) => {
